...
def search_url(base,page):
    query=urllib.parse.urlencode({'tag':'猫 犬','page':page,'mode':'safe','workType':'all','includeAi':'true'})
...
def main():
    ...
    env=os.environ.copy();env.update({'MOKU_RUNTIME_DIR':str(runtime),'MOKU_MUTEX_NAME':'Local\\MOKU.TagCache.'+os.urandom(12).hex(),'MOKU_NO_BROWSER':'1','MOKU_TEST_EXIT_AFTER_SECONDS':'300'})
